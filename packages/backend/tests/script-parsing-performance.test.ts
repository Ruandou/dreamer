...
describe("Script Parsing Performance Benchmark", () => {
  ...
  function generateTestScript(
    ...
    const scenes: ScriptScene[] = Array.from(
      { length: sceneCount },
      (_, i) => ({
        sceneNum: i + 1,
        location: `场景${(i % 5) + 1}`,
  ...
  function setupMockData(
    ...
    mockCharacterRepository.findManyByProject.mockResolvedValue(
      characters as any,
    );
    mockCharacterRepository.findManyByProjectNameAscWithImages.mockResolvedValue(
      characters as any,
    );
    mockCharacterRepository.findManyByProjectAndNames.mockResolvedValue(
      characters as any,
    );
    mockCharacterRepository.findImagesByCharacterIds.mockResolvedValue(
      characters.flatMap((c) => c.images) as any,
    );
    mockCharacterRepository.findFirstByProjectAndName.mockImplementation(
      (async (_proj: string, name: string) => {
    ...
    mockCharacterRepository.createCharacterImage.mockImplementation(
      async (data: any) => ({
        id: `img-new-${Date.now()}`,
        ...data,
      }),
    ...
    mockLocationRepository.findManyByProjectOrdered.mockResolvedValue(
      locations as any,
    );
  ...
  describe("Performance with different scales", () => {
    ...
    it("should verify batch operations are called (not N+1 queries)", async () => {
      ...
      expect(
        mockCharacterRepository.findManyByProjectAndNames,
      ).toHaveBeenCalled();
      expect(
        mockCharacterRepository.findImagesByCharacterIds,
      ).toHaveBeenCalled();

      // These should NOT be called repeatedly in a loop
      const findFirstCalls =
        mockCharacterRepository.findFirstByProjectAndName.mock.calls.length;
      console.log(`\n🔍 N+1 Query Check:`);
  ...
  describe("Optimization verification", () => {
    ...
    it("should use batch delete for alias characters", async () => {
      ...
      const mergeModule =
        await import("../src/services/ai/character-identity-merge.js");
    ...
    it("should use batch update for character descriptions", async () => {
      ...
      expect(
        mockCharacterRepository.updateManyCharacterDescriptions,
      ).toHaveBeenCalled();
      console.log(`\n✅ Batch Operations:`);
