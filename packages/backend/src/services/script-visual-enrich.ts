...
function buildDerivativePrompt(
  ...
  if (/同一人|与基础|保持一致|参考基础|与基础定妆|相同人物|同一人物/i.test(d))
    return d;
...
function sortCharacterImageSlots(slots: ParsedCharImage[]): ParsedCharImage[] {
  return [...slots].sort(
    (a, b) => slotProcessOrder(a.type) - slotProcessOrder(b.type),
  );
}
...
function resolveDbLocationName(
  ...
  const stripTrailingPeriod = (s: string) =>
    collapsed(s).replace(/[.。．]+$/u, "");
  ...
  const tryOne = (raw: string): string | null => {
    ...
    if (stripped !== t) {
      ...
      for (const n of dbNames) {
        if (
          stripTrailingPeriod(n) === st ||
          collapsed(n) === collapsed(stripped)
        )
          return n;
      }
    }
...
async function fillMissingCharacterImagePrompts(
  ...
  const characters =
    await characterRepository.findManyByProjectNameAscWithImages(projectId);
    ...
    for (const img of c.images) {
      ...
      const parentSlotSummary =
        img.parentId && img.type !== "base"
          ? c.images.find((i) => i.id === img.parentId)?.prompt?.trim() ||
            basePrompt
          : basePrompt;
...
export async function applyScriptVisualEnrichment(
  ...
  const projectRow =
    await projectRepository.findUserIdAndVisualStyle(projectId);
  if (!projectRow) {
    throw new Error(
      "视觉补全失败：项目不存在，无法写入模型审计日志与定场/定妆提示词",
    );
  }
  ...
  const locations =
    await locationRepository.findManyByProjectOrdered(projectId);
  const characters =
    await characterRepository.findManyByProjectNameAscWithImages(projectId);
  ...
  try {
    ...
    console.warn(
      "[applyScriptVisualEnrichment] 模型返回内容无法解析为 JSON:",
      err?.message || err,
    );
    throw new Error(
  ...
  if (Array.isArray(payload.locations)) {
    for (const loc of payload.locations) {
      ...
      const r =
        await locationRepository.updateManyActiveImagePromptByProjectAndName(
          projectId,
          resolvedName,
          prompt,
        );
  ...
  for (const pc of payload.characters) {
    ...
    const images = sortCharacterImageSlots(
      Array.isArray(pc.images) ? pc.images : [],
    );
    let baseImageId: string | null =
      character.images.find((i) => i.type === "base" && !i.parentId)?.id ||
      null;
    let lastBasePrompt: string | null =
      character.images
        .find((i) => i.type === "base" && !i.parentId)
        ?.prompt?.trim() || null;
    ...
    for (const slot of images) {
      ...
      if (type === "base") {
        const existingBase = character.images.find(
          (i) => i.type === "base" && !i.parentId,
        );
        if (existingBase) {
      ...
      if (isDerivedImageType(type)) {
        ...
        const maxOrder = await characterRepository.maxSiblingOrder(
          character.id,
          parentId,
        );
        const created = await characterRepository.createCharacterImage({
