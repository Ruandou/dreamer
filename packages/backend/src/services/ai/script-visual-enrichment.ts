...
export function buildScriptVisualEnrichmentUserContent(input: {
  ...
  return [
    ...
    '2. **base**：prompt 为完整中文定妆提示词（主参考图）。须：站立于**纯色影棚背景**（如中灰色）；**正面全身构图（头顶到脚底完整可见）**；四段意合为一段、句间用句号分隔，整体不超过约150字：（1）面部特征（脸型、眼睛、眉毛、鼻子、嘴唇）（2）整体外貌与发型（3）服装与姿态（4）构图与背景须显式包含正面全身与纯色底，并融入上方「项目视觉风格」与画质词；禁止只写剧情动作，须写「长什么样、穿什么」。',
