...
export class PromptRegistry {
  ...
  initialize(): void {
    ...
    console.log(
      `[PromptRegistry] Registered ${this.getAllTemplates().length} templates`,
    );
  }
  ...
  getByCategory(category: string): PromptTemplate[] {
    this.ensureInitialized();
    const allVersions =
      PromptTemplateEngine.getInstance().getByCategory(category);
  ...
  register(template: PromptTemplate): void {
    ...
    if (!this.initialized) {
      console.warn(
        `[PromptRegistry] Registering template "${template.id}" before initialization`,
      );
    }
  }
