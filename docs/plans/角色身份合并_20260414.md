---
name: 角色身份合并
overview: 在「导入解析」与「大纲解析剧本」两条链路上统一：让 DeepSeek 在落库前完成「同人异名」合并，输出嵌套 images 槽位；再规范化分集 script 中的角色名，并据此创建 Character / CharacterImage；同时保证定妆/文生图 prompt 能写入（视觉补全成功或兜底），避免角色槽位长期无图片生成提示词。
todos:
  - id: types-parser
  ...
  - id: normalize-persist
    content: 实现 normalizeScriptContent；runParseScriptJob 中写回各集 script 再 merge
    status: completed
...
| `Episode.script` 称谓替换 | `packages/backend/src/services/character-identity-normalize.ts`（`normalizeScriptContent`） |
...
**说明**：原计划第 4 节写「`saveCharacters` + `applyScriptVisualEnrichment`」在 Pipeline 里分步调用；落地时改为 **`runParseScriptEntityPipeline` 统一封装**（内含身份合并、写回 script、saveLocations、saveCharacters、槽位与默认 base），再 `applyScriptVisualEnrichment`，避免两条链路行为不一致。
...
因此需要 **两条链路都改**：导入侧改 parser + importer，并在导入成功后 **补一轮定妆 prompt**（或共用与解析剧本相同的补全函数）；大纲侧在 `saveCharacters` 之前增加「身份合并 + `Episode.script` 规范化 + 按槽位建图」，并处理 **prompt 兜底**（见 3.1）。
...
flowchart LR
  ...
  subgraph after [目标]
    M[DeepSeek 身份合并] --> N[规范化 script 称谓]
...
## 3. 大纲「解析剧本」：DeepSeek 身份合并 + `Episode.script` 写回
...
- **`Episode.script` 规范化**（[`character-identity-normalize.ts`](packages/backend/src/services/character-identity-normalize.ts)）：
...
- **写回分集**：[`runParseScriptJob`](packages/backend/src/services/project-script-jobs.ts) 经 [`runParseScriptEntityPipeline`](packages/backend/src/services/parse-script-entity-pipeline.ts)：对 `targetEpisodes` 范围内各 episode 读 `script` → 规范化 → [`episodeRepository.update`](packages/backend/src/repositories/episode-repository.ts) 写回；再 `mergeEpisodesToScriptContent`。
- **落库顺序（核心）**：
...
- **唯一约束**：`Character` 的 `@@unique([projectId, name])` 要求规范名全局唯一；合并后写回 script 可避免再次创建别名行。
